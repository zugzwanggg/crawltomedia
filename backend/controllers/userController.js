...
export const getAvailableApps = async (req,res) => {
  try {
    ...
    const apps = await db.query("SELECT * FROM apps LEFT JOIN user_apps ON user_apps.app_id = apps.id WHERE user_apps.user_id IS NULL OR user_apps.user_id != $1;", [user_id]);
